...
namespace GymMangement.BLL.Services.Classes
{
    public class SessionService : ISessionService
    {
        ...
        public async Task<IEnumerable<SessionViewModel>?> GetAllSessionsAsync(CancellationToken ct = default)
        {
            ...
            var sessions = await sessionRepo.GetAllSessionswithTrainerAndCategory(ct:ct);
        ...
        public async Task<Result<UpdateSessionViewModel>> GetSessionToUpdateAsync(int sessionId, CancellationToken ct = default)
        {
            ...
            var bookingCount = await _unitOfWork.SessionRepository.GetCountOfBookedSlotAsync(sessionId, ct);
            if(bookingCount > 0)
        ...
        public async Task<Result> UpdateSessionAsync(int id, UpdateSessionViewModel model, CancellationToken ct = default)
        {
            ...
            var BookedCount = await _unitOfWork.SessionRepository.GetCountOfBookedSlotAsync(id);
            if (BookedCount > 0)
        ...
        public async Task<Result> RemoveSessionAsync(int sessionId, CancellationToken ct = default)
        {
            ...
            if (session.EndDate >= DateTime.Now)
                return Result.Fail("Can Not Delte Session That Has Not Ended Yest!");
            var BookedCount = await _unitOfWork.SessionRepository.GetCountOfBookedSlotAsync(sessionId, ct);
            if (BookedCount > 0)
